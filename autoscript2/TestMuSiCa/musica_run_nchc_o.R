...
vcfdata=as.vector(unlist(strsplit(cases,":")))
vcfdatapath=c()
for (i in vcfdata){ 
  d=paste0(datafolder,"/",i)
  vcfdatapath<-append(vcfdatapath,d)
}
print(vcfdatapath)


inFile<-data.frame(
  datapath = vcfdatapath,
  name = vcfdata,
  size = c(NA),
  type = c(NA),
...
auxfile <- paste0(programfolder,"/aux_files/proposed_etiology_COSMIC_signatures.txt")
auxfile1 <- paste0(programfolder,"/aux_files/cancermatrix.tsv")
proposed_etiology <- fread(auxfile,sep="\t",header=F,data.table=F)[,2]
known_cancer_signatures<-read.table(auxfile1,header=TRUE,sep="\t",row.names=1)
